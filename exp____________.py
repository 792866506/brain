...
from braindecode.experiments.stopcriteria import MaxEpochs, ColumnBelow, Or,NoDecrease
...
class Experiment(object):
    ...
    def run_one_epoch(self, datasets, remember_best,batch_modifier):
        ...
        if remember_best:
            self.rememberer.remember_epoch(self.epochs_df, self.model,
                                           self.optimizer)
    def run_second_epoch(self, datasets, remember_best):
        """
        Run training and evaluation on given datasets for one epoch.
    ...
    def setup_after_stop_training(self):
        ...
        #self.rememberer = RememberBest('test_misclass')
